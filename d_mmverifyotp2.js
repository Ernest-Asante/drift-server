...
router.post('/', async (req, res) => {
  const { identity, otp, contact, firstName, lastName , ghanacard, address, photo} = req.body;
  ...
  try {
    ...
    try {
      const { data: insertData, error: insertError } = await supabase
        .from('driver')
        .update({ [userColumn]: contact, fname: firstName, lname: lastName, verified: true, [`${userColumn}_verify`]: true , national_id: ghanacard, address: address, imageurl: photo})
        .eq(identityColumn, identity);
