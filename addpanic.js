...
router.post('/', async (req, res) => {
  const { identity, firstName, lastName, relation, contact } = req.body;

  try {
    const {data: insertData, error: insertError } = await supabase
      .from('rider')
      .update({panic_fname: firstName, panic_lname: lastName,panic_phone: contact, panic_relation: relation, panic_included: true, })
